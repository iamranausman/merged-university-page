...
const CourseDetailPage = ({s_course}) => {
  ...
  useEffect(() => {
    try
    {
      setLoading(true);
      if(!s_course.success){
        throw new Error(s_course.message);
      }

      if(s_course.success){
        setCourse(s_course.data);
        setRelatedCourses(s_course.relatedcourse || []);
      } else {
      ...
      <Container>
        <div className="complete-page banner-bottom-space bottom-session-space complete-page-spaceing">
          ...
          <div className="flex gap-[10px] mt-4 justify-center">
            ...
            {/* RIGHT SIDE IMAGE */}
            <div className="">
              <Image src={course.campusImage && course.campusImage.trim() !== '' ? course.campusImage : '/assets/visit.webp'} alt="campus" width={575} height={604} className="w-full h-full object-cover" />
            </div>
          </div>
          <div className="">
