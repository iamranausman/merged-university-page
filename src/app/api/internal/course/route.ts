...
export async function GET(request: NextRequest) {
  try {
    ...
    if (qualification && qualification !== '') {
      ...
      console.log("Check qualification split", qualification.split(',').map(q => q.trim()).filter(q => q))
      if (qualificationList.length > 0) {
        const qualificationFilters = qualificationList.map(qual => `(qualification LIKE ?)`).join(' OR ');
        whereClause += ` AND (${qualificationFilters})`;
        qualificationList.forEach(qual => params.push(`%${qual}%`));
      }
    }
...
export async function POST(req: NextRequest) {
  ...
  try {
    ...
    const [checkUniversity] = await pool.query<RowDataPacket[]>("SELECT * FROM university_details WHERE id = ?", [data.university_id])

    if(checkUniversity.length === 0){
      return NextResponse.json(
        {
          success: false,
          message: "You select an invalid Univeresity. Please select the correcrt one",
        },
        {
          status: 400
        }
    ...
    const [checkSubject] = await pool.query<RowDataPacket[]>("SELECT * FROM subjects WHERE id = ?", [data.subject_id])

    if(checkSubject.length === 0){
      return NextResponse.json(
        {
          success: false,
          message: "You select an invalid Subject. Please select the correcrt one",
        },
        {
          status: 400
        }
    ...
    const [checkLevel] = await pool.query<RowDataPacket[]>("SELECT * FROM posts WHERE id = ?", [data.qualification])

    if(checkLevel.length === 0){
      return NextResponse.json(
        {
          success: false,
          message: "You select an invalid Level of Qualification. Please select the correcrt one",
        },
        {
          status: 400
        }
    ...
    const [result] = await pool.query<ResultSetHeader>(
      `INSERT INTO courses (
        name, university_id, subject_id, sm_question, sm_answer, review_detail,
        rating_count, review_count, avg_review_value, qualification, duration,
        duration_qty, duration_type, yearly_fee, application_fee, languages,
        starting_date, deadline, about, entry_requirments, curriculum,
        scholarship, sort_order, active, display, popular
      ) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`,
      [
        data.name,
        data.university_id,
