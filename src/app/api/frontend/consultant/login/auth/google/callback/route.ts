...
export async function GET(req: NextRequest) {
  ...
  try {
    ...
    if(checkEmail.length > 0)
    {
      ...
      {
        return NextResponse.redirect(`${APP_URL}/consultant-login?error=affectedrows`);
      }
      ...
      const response =  NextResponse.redirect(`${APP_URL}/consultant-dashboard`);
      ...
      const [newUser] = await pool.query<ResultSetHeader>("INSERT INTO users (first_name, email, image, provider, googleid, user_type) VALUES (?, ?, ?, 'Google', ?, 'consultant')", [data.name, data.email, data.picture, data.id])
    ...
    console.error(error);
    return NextResponse.redirect(`${APP_URL}/consultant-login?error=google`);
  }
}
