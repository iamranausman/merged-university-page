  ...
  try {
    ...
    const newVisa = await prisma.visit_visas.create({
      data: {
        ...
        email: body.email,
        phone: body.mobile,
        last_education: body.education,
        gender: genderMap[body.gender?.toLowerCase()] || null,
        taxpayer_type: body.taxpayer,
        bank_statment: body.bankStatement,
        country: body.homeCountry,
        state: body.state,
        city: body.city,
        country_name: body.applyFor,
    ...
    await resend.emails.send({
      ...
      subject: 'Visit Visa Application Created',
      html: `<p>Visa application from: ${body.givenName || 'Unknown'} (${body.email || 'N/A'})</p>`
    });
...
export async function GET(req) {
  try {
    ...
    const [total, records] = await Promise.all([
      prisma.visit_visas.count({ where }),
      prisma.visit_visas.findMany({
        where,
        orderBy,
    ...
    return Response.json({ success: true, data: records, total, page, pageSize });
  } catch (error) {
...
export async function PATCH(req) {
  try {
    ...
    if (!id) {
      return Response.json({ success: false, message: 'ID is required for update' }, { status: 400 });
    }
    const record = await prisma.visit_visas.findUnique({ where: { id: parseInt(id) } });
    if (!record) {
    ...
    const updated = await prisma.visit_visas.update({
      where: { id: parseInt(id) },
      data: updateFields,
    });
    return Response.json({ success: true, data: updated });
  } catch (error) {
    console.error('PATCH /api/visit-visas error:', error);
    return new Response(
      JSON.stringify({ success: false, message: 'Update failed', error }),
      { status: 500 }
    );
  }
}