...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    if(decoded){
        ...
        {
            const [againcheck] = await pool.query<RowDataPacket[]>("SELECT * FROM users WHERE id = ? AND user_type = ? ", [decoded.id, 'student']);
            ...
            const [updatePassword] = await pool.query<ResultSetHeader>("UPDATE users SET password = ? WHERE id = ? AND user_type = ?", [hashedPassword, decoded.id, 'student'])
